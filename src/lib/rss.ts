...
const demoItems: FeedItem[] = [
  {
    ...
    summary:
      'Hardware vendors are shifting attention toward lightweight assistants that handle quick summaries and notifications directly on the device.',
    link: '#',
  ...
  },
  {
    ...
    summary:
      'Product teams are redesigning home screens so readers start with a compact briefing before opening full stories.',
    link: '#',
...
function getText(parent: Element, selectors: string[]) {
  ...
  for (const element of descendants) {
    ...
    if (
      normalizedSelectors.includes(tagName) ||
      normalizedSelectors.includes(localName)
    ) {
...
function normalizeItem(sourceFeed: FeedConfig, item: Element, index: number): FeedItem {
  ...
  return {
    ...
    summary: summary || 'Die Quelle hat keine Zusammenfassung für diesen Eintrag geliefert.',
    link: getLink(item),
...
export async function loadConfiguredFeeds(feeds: FeedConfig[]): Promise<FeedLoadResult> {
  if (feeds.length === 0) {
    return {
      items: [],
      health: [],
      message: 'Füge einen Feed hinzu, um deinen Digest aufzubauen.',
    }
  }
  ...
  return {
    ...
    message:
      'Die Live-Feeds waren nicht erreichbar, deshalb wird ein Demo-Digest angezeigt. Pruefe die gespeicherten Feed-URLs oder die aktive Proxy-Konfiguration.',
  }
}
